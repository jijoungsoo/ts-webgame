import {Card,Player,Sub,Hero} from './types'
...
function createHero({mine}:{mine:boolean}){  //파리미터의 타입은 : 뒤에 써준다.  {mine:boolean,count:number}
    const player = mine? me:opponent;
...
interface A {
    data:Card,DOM:HTMLDivElement,hero?:boolean
}

function connectCardDOM({data,DOM,hero=false}:A){  //파리미터 타입을 인터페이스로 바꿔줄수있다.
